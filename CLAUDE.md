...
**Agent-language discipline (LOCKED 2026-06-24 — MIL-185 decision (a), honest reframe).** MIL/Hodos is a **governed deterministic intelligence pipeline + adapters, with an evidence-grounded chat layer** — NOT an autonomous agent swarm. The earlier *"swarm of agents that becomes the enterprise's decision-intelligence layer"* framing (captured 2026-05-06) is **aspirational positioning with no runtime referent today**: the pipeline is cron-driven functions with no agent loop, no tool-use planning, no inter-agent messaging, no autonomy budget; the "10 implicit agents" are pipeline stages, not agents. Why honest framing wins (agentic-AI panel, 2026-06-23): determinism + per-decision explainability is exactly what 2026 regulation rewards, and "deterministic governed pipeline" survives technical due diligence where "agent swarm" invites the *Agent-as-Business-Process Fallacy* critique. **Rule:** in external/buyer/HODOS-2 materials describe it as a governed deterministic pipeline; only claim "agents" once a genuine agent loop (plan → act → observe with a step budget) actually ships. Build option (b) — one real Researcher agent loop — stays available if the swarm narrative ever becomes commercially load-bearing, but is not the default. Dated session logs that use the old phrasing are historical record, not current framing.
...
Next ticket: PULSE-146 (highest filed PULSE-145 — 2026-06-23 agentic-AI panel slate: **PULSE-143** TOST synthetic→real transfer-eval scaffold + **PULSE-144** REG-001..004 production-gates DONE 2026-06-24; **PULSE-145** P20/P21 hard-fail-on-high-stakes FILED.) (**Session 2026-05-24/25 — taxonomy correction + themes-as-decision-packs.** Verified against taq-app `inventory/screens.yaml`: the real taxonomy is **Journey (24, `journey` field, a backend/technical grouping — NOT customer journeys) / Customer Journey (107, `feature` field, CJ01–CJ107, the roll-up) / Op-code (697, the grain)**; 97 op-codes have no Customer Journey → orphan bucket + coverage (86.08%). Filed **PULSE-137** (engine taxonomy realignment 18-journey→real 24/107/697; **increment 1 BUILT on branch `feat/pulse-137-taxonomy-realignment` @ `587265c`, 596 pulse tests pass, NOT yet on main; increment 2 = consumer migration, deferred** — v1 detection stays on synthetic friction-screens), **HOL-88** (Row-2 op-code filter + progressive-drill content). **THEMES = DECISION PACKS (LOCKED)** — same unit, re-keyed from `(screen × signature)` cells to the decision-output/theme axis → **PULSE-138** (rebase decision-pack registry/moat onto themes; migrate the 12 cell-packs to leaf evidence), **PULSE-139** (rebase FrictionBench as the eval substrate under themes), **PULSE-136** = theme engine that *produces* the theme-packs (commented to converge). Follow-ons FILED: **PULSE-140** (synthesis re-target, PULSE-93 → per-theme) + **HOL-89** (UI navigation re-founding, retire `?pack=`). Memory: `project_pulse_surface_ia_and_verification.md` + next-session pickup `project_next_session_pulse_theme_rebase.md`.) (**PULSE-132/133/134 DONE 2026-05-24** — MLOps-monitor engine substrate, all additive + default-off so the deterministic single-cohort corpus is byte-identical (588 pulse tests): PULSE-132 = real fairness verdict (`convergence.assess_fairness`) in the Cause analytics payload; PULSE-133 = `pulse/serving/run_history.py` run-history persistence + controlled corpus drift (`generate_corpus(drift_pct=…)` → `degraded_positive` recall degradation); PULSE-134 = mixed-cohort corpus (`mixed_cohorts`/`cohort_disparity`) + per-run fairness history. KEY LEARNING: the synthetic FrictionBench corpus is single-cohort-per-cell + deterministic BY DESIGN, so drift/fairness monitor demos need these scenario knobs — see memory `feedback_synthetic_corpus_scenario_knobs.md`.) (PULSE-130 = behavioral-sequence Transformer spike, **off-node research only** — bank edge node confirmed CPU-only + no `accelerate`, so deep-model training stays off-node; reference artifact `pulse/seq/` uses a native PyTorch loop, not HF `Trainer`; PR #5 on `feat/pulse-130-seq-spike`; APPROVED_LIBRARIES.md captures torch/transformers/duckdb pins (+ holter mirror). PULSE-131 = the **in-bank, on-mission** path = classical sequence model (DuckDB within-session transition model + features → scikit-learn), which fits the non-LLM/classical runtime — next session builds `pulse/seq/transitions.py`. MIL-176 = pre-existing `clone_doctor.py` Zero-Entanglement validator violation (whitelist/refactor). PULSE-129 reserved. PULSE-128 = engine relocation into while-sleeping, 2026-05-22; the full Pulse vertical — pipeline → decisions → lineage → fairness → audit → CHRONICLE — built under PULSE-128 with PULSE-28/34/39 re-scoped to DuckDB. PULSE-110/113-127 also exist from the holter data-layer/detection work. "PULSE-1 through PULSE-91" above is design-spine history, not the live high-water mark.)
Board: Scrum
...
Next ticket: MIL-187 (highest filed MIL-186 — 2026-06-24 agentic-AI-panel + ops slate: MIL-179 date-floor-backfill (won't-build, iTunes-RSS 500 ceiling), MIL-180 GPlay-date fix DONE, MIL-181 model-IDs→Opus-4.8 DONE, MIL-182 reranker DONE, MIL-183 BGE embedding DONE, MIL-184 verifier fail-closed DONE (gold-set half OPEN), MIL-185 agent-language reframe DONE, MIL-186 durable run journal DONE. Older context: MIL-171 (highest filed: MIL-170 — MIL-165 is the canonical /journey-page ticket; MIL-166 mirrors MIL-165; MIL-167/168/169/170 are placeholder reservations from the 2026-04-30 MCP write-loop incident, kept open for future repurposing)
...
Next ticket: HODOS-5 (HODOS-2 architecture-doc DRAFTED 2026-06-24 from the kernel spike + research, then tidied off main with the experiment; HODOS-3 = "Hodos contracts ↔ MCP/A2A" mapping (filed, board-gated on HODOS-1); HODOS-4 = Hodos kernel SPIKE — built, proved one-engine-many-products, then TIDIED OFF main per Hussain "just an experiment", marked Done. **Hodos is NOT being built** — see memory project_session_2026_06_24_panel_hodos. HODOS-1 panel ran 2026-06-24: recommendations recorded, NOT ratified.)
...
Next ticket: HOL-95 (**HOL-94 DONE 2026-06-26** — Cerno **Exploration** surface: 3-tab IA fold (Decisions / Intelligence / **Exploration**; Friction + Verification merged via an Explore/Verify segmented control in `holter/preview/render_exploration.py`; rank-1 HERO + compact context band in `render_cerno.py`; `/cerno`→`/exploration` redirect; `/mlops` keeps the full console, not in nav). UX design-panel R1 7.42 → R2 8.13 (lock-eligible); caught + fixed a broken main; while-sleeping PR #56 + cjipro/holter PR #4 merged; 154 holter tests. **CERNO → COPILOT HANDOFF PUBLISHED 2026-06-26 (P5-clean):** code = `github.com/cjipro/holter` + `app.cjipro.com/engineering/cerno-code` (pasteable skeleton; generator `mil/auth/app_cjipro/scripts/gen_cerno_code_page.py`); build prompt = `app.cjipro.com/engineering/cerno-build`. **NEXT = Copilot builds Cerno on the work machine (real Step-4 D-014 / marts) → executive progress demo** — see memory `project_next_session_cerno_copilot_handoff`. **HOL-90 / HOL-91 / HOL-92 also DONE** (Cerno-in-Holter-design / cjipro re-sync / doc reconcile). **HOL-93** Cerno Intelligence-tab box-overlap fix DONE 2026-06-25: kept the mobile-screen box height, scrolled `.box-body` inside it, 3-up rows; scoped to `main.holter-main`.) (**HOL-88 filed 2026-05-24** — Row-2 Op-code filter + progressive-drill content: add Op-code as a searchable, multi-select, cascade-narrowed 3rd Row-2 filter (Customer Journey → Sub-Journey → Op-code → Sort); content shows children ranked by Friction+Opportunity score at rollup levels, raw counts (# errors/NPS/calls + # sessions + # customers) at the op-code leaf; theme carries down; extends HOL-86, depends on PULSE-137 + PULSE-136. Follow-on FILED as **HOL-89**: re-found UI navigation on themes (retire `?pack=` / pack-as-navigation, drill supersedes the HOL-78 altitude switcher). See memory `project_pulse_surface_ia_and_verification.md`.) (**HOL-85 / HOL-83 / HOL-87 / HOL-86 DONE + MERGED 2026-05-24** — the two-row selector IA, shipped to `main` @ `5c7a9d9` (PRs #34/#36/#38 merged; #35/#37 closed-cosmetically with code in main). **HOL-85** Row-1 Themes context selector (config-driven `holter/themes.yaml`, 13 themes incl. Disputes/Fraud placeholders gated, `?theme=` persists globally). **HOL-83** surfaces reframed Home/Workspace/MLOps → **Decisions/Intelligence/Verification** + MLOps vocabulary stripped from the Verification surface (deploy-action cluster removed, "model-deployment risk → deploying DS team" boundary line, panes → FINDING RELIABILITY / JOURNEY FAIRNESS / FINDING LINEAGE / SYNTHESIS PROVENANCE). **HOL-87** marquee moved Intelligence→Decisions (fixed a flex-column 1px-collapse). **HOL-86** global Row 2 selector (Journey ▾ multi-select+search from `journey_taxonomy.yaml` · Sub Journey ▾ gated · Sort Friction/Opportunity) injected on all three surfaces by `server.py::_page` + legacy Product/Owner filter strip removed. Both rows are now global chrome on every surface; 140 holter tests; in-browser verified. **Selectors are SHELLS until PULSE-136** (theme engine) wires real content + the Row-2 cascade. State memory: `project_pulse_surface_ia_and_verification.md`. — Earlier 2026-05-24: **HOL-76 / HOL-77 / HOL-78 DONE 2026-05-24** — full-page Flask front-end (`holter/server.py`, `:8600`) UI arc: **HOL-76** Home→Workspace drill-through (Home cards deep-link to `/workspace?pack=<name>`, in-page, no new tab; retired the dead `_STALE_LINK` server workaround); **HOL-77** MLOps+Home bounded-shell scroll fix (`body` becomes a 100vh flex shell + page-`main` an `overflow-y:auto` region, `:has()`-scoped so the Workspace `.holter-app` shell is untouched); **HOL-78** Workspace altitude switcher (full-width Bank/Journey/Signal band rendering each pack's real `samples/{altitude}.md` via a new dependency-free markdown→HTML converter `holter/preview/_md.py`, default Journey, honors the `?pack=` deep-link). PRs #27/#28/#29 merged; 98 holter tests; verified in-browser via chrome-devtools. State memory: `project_holter_mlops_wiring.md`. **HOL-3 / HOL-4 / HOL-6 + HOL-74 + HOL-75 DONE 2026-05-24** — Holter UI surfaces wired to the live Pulse engine via fail-soft bridges in `holter/preview/` (`get_pack_analytics`, `_drift_series_by_cell`, `_fairness_history_by_cell`, `_lineage_report`; consume the public `pulse.*` API only — Zero-Entanglement intact). Workspace (HOL-3) + Home (HOL-4) show real confidence band/interval + lineage anchor; **MLOps Console fully real across all 4 panes** — synthesis-mode governance from pack metadata (HOL-6), lineage (global decision-chain) + fairness (real verdict) + drift (per-cell run-history) (HOL-74), fairness over-time trend (HOL-75). Every pane has an honest NO-DATA / not-assessable state where the synthetic substrate is limited — no fabricated numbers on the procurement gate. PRs #16–#25 merged. State memory: `project_holter_mlops_wiring.md`.)
...
- **Cerno** = bank-facing **top-bar display label only** (HOL-84, 2026-05-24). The text on the Holter top bar reads "Cerno". **This is a label, nothing else — no architectural value.** It does NOT rename the engine (stays Pulse / `pulse`), the UI (stays Holter), the CJI brand or four-product family, Hodos, repos, packages, Jira keys, or URLs. Presentation framing only: "Cerno, powered by Pulse (engine) and Holter (UI)." Do not over-interpret as a rebrand. (Aspirational product direction — Decisions/Intelligence/Verification surfaces + Verification framework — lives in memory `project_pulse_surface_ia_and_verification.md`, NOT triggered by this label.)
...
- **RAG: embedding cosine similarity (2026-04-18)** — replaced keyword overlap with `all-MiniLM-L6-v2` sentence embeddings. CHR embeddings cached in `_CHR_EMBED_CACHE` at startup; each signal text encoded once, cosine sim computed against all 19 CHR vectors. Keyword overlap retained as fallback only. **Pinned `device="cpu"` 2026-05-24** (`rag.py` `_load_embed_model`): this in-process GPU encode was the op in flight at 3 mid-pipeline BSODs (`0x116` VIDEO_TDR_ERROR, RTX 5070 Ti Laptop); CPU cost is negligible for this tiny model and removes the GPU as a daily-run crash trigger. See memory `project_gpu_tdr_crashes.md`. **GPU driver updated 577.13→591.74 (CUDA 13.1) + validated 2026-05-25 — pipeline run #98 plus a sustained torch + Ollama stress test produced zero `0x116`/TDR; `MIL Daily` re-enabled (next fire 06:30 UTC). CPU pin retained as belt-and-braces.**
