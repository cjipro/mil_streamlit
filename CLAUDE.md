...
- **Apr 25 — website-chain admin steps (none blocking ship)**:
  - **MIL-82 DNS reservations** — DONE 2026-04-25. CNAMEs `app` + `admin` → `login.cjipro.com` (proxied) live in Cloudflare. `curl -sI https://app.cjipro.com/` returns CF TLS + HTTP 522 (no Worker bound yet — expected, Mode B placeholder). MIL-84 deploy auto-replaces; MIL-83 deploy auto-replaces.
