...
| MIL-49 | Sonar PDB email distribution — `mil/notify/briefing_email.py`, Opus lede + Haiku verifier, immutable subject, silent-day guard | BUILT 2026-04-22 (commit 9fc6116). Subject reframed + Dear Team greeting 2026-04-23 (6fee6fe). Verifier parser fail-safe + max_tokens fix 2026-04-23 (a021612). |
...
### MIL-50 — Public landing page + cjipro.com domain unblock (BUILT 2026-04-23, commit 1d9c2c9)
...
- **Apr 23 DONE (Jira tickets MIL-50..54 created)**: MIL-50 Public landing page + cjipro.com domain unblock (BUILT — close in UI). MIL-51 URL-filter vendor categorisation submissions (IN_PROGRESS — Cisco Talos submitted 2026-04-23, Zscaler / Palo Alto / Forcepoint / Symantec pending). MIL-52 Gmail Send-as for hello@cjipro.com + SPF update (BACKLOG). MIL-53 Track Ask CJI Pro v1 codebase in git (BACKLOG, hygiene, load-bearing — `mil/chat/` + `app/pages/08_ask_cji_pro.py` + `mil/command/ask_page.py` + `ops/cloudflared/` + 4 Ask CJI Pro routes in `model_routing.yaml` all still untracked). MIL-54 Custom team domain for sonar.cjipro.com (BACKLOG, minimal description — MCP WAF-blocked the fuller draft; see session memory `project_session_2026_04_23.md` for full scope). MIL-50's description cross-references are off by one due to MCP retry reshuffling Jira numbers — non-load-bearing, fix in Jira UI if convenient.
- **Apr 28 (current autonomy target)**: Task Scheduler fires `run_daily.py` at 06:30 UTC automatically. Ask CJI Pro v1 ready for alpha partners — next step is (a) MIL-54 custom team domain so `sonar.cjipro.com` is reachable on Barclays corp network, (b) MIL-48 partner provisioning, (c) MIL-52 Gmail Send-as so replies come from `hello@cjipro.com`. Alternative pivot: CJI Pulse PULSE-11 unblock.
