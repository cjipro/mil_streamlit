...
| MIL-61 | Cloudflare Worker Edge Bouncer (JWT cookie check + route whitelist + WorkOS redirect) | BUILT 2026-04-24 — `mil/auth/edge_bouncer/`, TypeScript Worker using `jose` for JWT verification against WorkOS JWKS, `ENFORCE=false` shadow-mode default, 17/17 vitest tests passing, typecheck clean; deployment deferred until MIL-63 sets real session cookies |
