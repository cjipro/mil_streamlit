...
- **Apr 28 (current autonomy target)**: Task Scheduler fires `run_daily.py` at 06:30 UTC automatically. Ask CJI Pro v1 ready for alpha partners — next step is (a) MIL-54 custom team domain so `sonar.cjipro.com` is reachable on Barclays corp network, (b) MIL-48 partner provisioning, (c) MIL-52 Gmail Send-as so replies come from `hello@cjipro.com`, (d) MIL-53 remainder — commit `mil/chat/` tree + ops scripts to git, sort `.gitignore` for runtime logs. Alternative pivot: CJI Pulse PULSE-11 unblock.
